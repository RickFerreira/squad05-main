...
import CampeonatoIndexStore from '../../stores/campeonato/indexStore';
...
@observer
class CampeonatoIndexPage extends GenericIndexPage {
  constructor(props) {
    super(props);
    this.store = new CampeonatoIndexStore();
  }
  ...
  render() {
    const columns = [
      ...
      {
        field: 'confederacaoOrganizadora',
        header: 'Confederação Organizadora',
      },
      {
        field: 'ano',
        header: 'Ano',
      },
      {
        ...
        body: ({ dataInicio }) => getValueDate(dataInicio),
      },
      {
        field: 'dataFim',
        header: 'Data de Fim',
        body: ({ dataFim }) => getValueDate(dataFim),
      },
      {
        style: { width: '110px' },
        body: (rowData) => {
          return (
            <div className="actions">
              <Link to={UrlRouter.campeonato.editar.replace(':id', rowData.id)}>
    ...
    const header = (
      <div className="table-header">
        <Link to={UrlRouter.campeonato.novo}>
    ...
    const breacrumbItems = [{ label: 'Campeonato' }];
...
export default CampeonatoIndexPage;
